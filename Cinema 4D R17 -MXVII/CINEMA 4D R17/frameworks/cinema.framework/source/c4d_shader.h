...
enum INITRENDERFLAG
{
	INITRENDERFLAG_0							= 0,						///< No flags.
	INITRENDERFLAG_TEXTURES				= (1 << 0),			///< Use textures.
	INITRENDERFLAG_PAINTERNOMIP		= (1 << 1),			///< Disable MIP for painting.
	INITRENDERFLAG_NOMIP					= (1 << 2),			///< Disable MIP.
	INITRENDERFLAG_PREVIEWRENDER	= (1 << 3),			///< Preview render.\n
																								...
																								/// }
																								/// @endcode
	INITRENDERFLAG_IRR						= (1 << 4)			///< Interactive region render.
...
/// @link SetSecond()@endlink/@link GetSecond()@endlink determine for a quadrangle which part of the quadrangle was stored (A-B-C or A-C-D).\n
/// @note	- Instead of <tt>if (lhit==0)</tt> write <tt>if (lhit.Content())</tt>
...
public:
	...
	Ray*				tray;										///< [READ WRITE] The transmitted vector (can be modified in MaterialData::ChangeNormal()), only valid if shader sets @ref VOLUMEINFO_TRANSPARENCY and only during MaterialData::CalcSurface().
	Ray*				rray;										///< [READ WRITE] The reflected vector (can be modified in MaterialData::ChangeNormal()), only valid if shader sets @ref VOLUMEINFO_REFLECTION and only during MaterialData::CalcSurface().
	...
	Vector			ddu,										///< [READ ONLY] U direction vector perpendicular to @ref n; can be used for bump mapping if @ref VOLUMEINFO_DUDVREQUIRED is set.
							ddv;										///< [READ ONLY] V direction vector perpendicular to @ref n; can be used for bump mapping if @ref VOLUMEINFO_DUDVREQUIRED is set.
	...
	Int32				calc_mip;								///< [READ ONLY] Specifies if MIP calculations will be done.\n
																			///< If this value is @em 0 then no MIP mapping needs to be calculated. @ref calc_mip can in some cases be false, even if the shader sets @ref VOLUMEINFO_MIPSAT.\n
	...
	Vector			pp[3];									///< [READ ONLY] MIP corner points in global space.
																			///< @note Only valid if @ref VOLUMEINFO_MIPSAT is set.
	...
	Vector			back_p;									///< [READ ONLY] The back-projected point (StickTextureTag) will be generated if the shader sets the flag @ref VOLUMEINFO_TRANSFORM or @ref SHADERINFO_TRANSFORM for a channel shader.
	Vector			back_delta;							///< [READ ONLY] This the 'spot size' (MIP mapping value) for the back-projected point.\n
																			///< The back-projected point (StickTextureTag) will be generated if the shader sets the flag @ref VOLUMEINFO_TRANSFORM (or @ref SHADERINFO_TRANSFORM if it is a channel shader).
	...
	Int32				unused[5];							///< [READ ONLY] Currently unused. @since R16.038
...
#define MSG_SHADERV5TOV6		112005			///< @markPrivate
#define MSG_SHADERV6TOV7		112006			///< @markPrivate
#define MSG_SHADERREAD			112009			///< @markPrivate
...
//----------------------------------------------------------------------------------------
/// Private message struct for the @ref MSG_SHADERREAD message.
//----------------------------------------------------------------------------------------
