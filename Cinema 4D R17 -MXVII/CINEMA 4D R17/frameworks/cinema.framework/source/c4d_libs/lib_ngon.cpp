...
Bool Ngon::Alloc(Int32 cnt, Int32 scnt)
{
	...
	if (cnt)
	{
		points = NewMemClear(Int32, cnt);
		if (!points) return false;

		if (scnt)
		{
			segments = NewMemClear(Int32, scnt);
			if (!segments)
			{
				DeleteMem(points);
				return false;
...
Bool Ngon::CopyFrom(const Ngon *src)
{
	...
	{
		points = NewMem(Int32, src->count);
		if (!points) return false;

		if (src->segments && src->segcount)
		{
			segments = NewMem(Int32, src->segcount);
			if (!segments)
			{
				Free();
				return false;
...
Bool Ngon::CopyFrom(Int32 cnt, Int32 scnt, Int32* pts, Int32* segs)
{
	...
	if (pts && cnt)
	{
		points = NewMem(Int32, cnt);
		if (!points) return false;

		if (segs && scnt)
		{
			segments = NewMem(Int32, scnt);
			if (!segments)
			{
				Free();
				return false;
		...
		{
			segcount = 1;
			segments = NewMem(Int32, segcount);
			if (!segments)
			{
				Free();
				return false;
...
Float Ngon::CalcArea(Int32 seg, const Vector* p, Matrix& m)
{
	...
	if (seg == 0)
	{
		...
		{
			m.v1 = Cross(Vector(0.0f, 1.0f, 0.0f), m.v3); // x axis
			m.v2 = Cross(m.v3, m.v1); // y axis
		}
		else if (Abs(n.y) > Abs(n.z))
		{
			m.v1 = Cross(m.v3, Vector(0.0f, 0.0f, 1.0f));
			m.v2 = Cross(m.v3, m.v1);
		}
		else
		{
			m.v2 = Cross(m.v3, Vector(1.0f, 0.0f, 0.0f));
			m.v1 = Cross(m.v2, m.v3);
		}
		m = ~m;
	}
