...
struct GeRayColResult
{
		...
		Vector	s_normal;					///< The phong normal at the intersection. (Not normalized.)
...
class GeRayCollider
{
	...
	public:
		...
		/// @param[in] force							If @formatConstant{false} then @C4D checks if the passed object's @c GetDirty(DIRTYFLAGS_DATA) is unchanged. If yes, it does nothing and returns @formatConstant{true}.\n
