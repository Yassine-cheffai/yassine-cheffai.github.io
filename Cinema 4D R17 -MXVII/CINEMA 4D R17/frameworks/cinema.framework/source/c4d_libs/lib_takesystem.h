...
enum TAKE_MODE
{
	TAKE_MODE_MANUAL	= 0,			///< Manual override: the user has to explicitly override every single parameter.
	TAKE_MODE_AUTO							///< Auto override: the system overrides any edited parameter (similar to Auto Key).
} ENUM_END_LIST(TAKE_MODE);
/// @}
...
enum OVERRIDEENABLING
{
	...
	OVERRIDEENABLING_CAMERA									= (1 << 9),				///< Active camera override enabled.
	OVERRIDEENABLING_RS											= (1 << 10),			///< Active render settings override enabled.
	OVERRIDEENABLING_PARAM									= (1 << 11)				///< Generic parameter override enabled.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Retrieves the Take System global mode, can be @ref TAKE_MODE_MANUAL or @ref TAKE_MODE_AUTO.
...
//----------------------------------------------------------------------------------------
/// Stop the Take render if it is running.
//----------------------------------------------------------------------------------------
void StopTakeRender();
