...
enum SCULPTBRUSHID
{
	SCULPTBRUSHID_SMOOTH		= 1024170,			///< Smooth brush ID.
	SCULPTBRUSHID_PULL			= 1024168,			///< Pull brush ID.
	SCULPTBRUSHID_FLATTEN		= 1024169,			///< Flatten brush ID.
	SCULPTBRUSHID_PINCH			= 1024171,			///< Pinch brush ID.
	SCULPTBRUSHID_ERASE			= 1024189,			///< Erase brush ID.
	SCULPTBRUSHID_GRAB			= 1024197,			///< Grab brush ID.
	SCULPTBRUSHID_MASK			= 1024506,			///< Mask brush ID.
	SCULPTBRUSHID_KNIFE			= 1026519,			///< Knife brush ID.
	SCULPTBRUSHID_WAX				= 1026518,			///< Wax brush ID.
	SCULPTBRUSHID_INFLATE		= 1026708,			///< Inflate brush ID.
	SCULPTBRUSHID_FILL			= 1026710,			///< Fill brush ID.
	SCULPTBRUSHID_SCRAPE		= 1026715,			///< Scrape brush ID.
	SCULPTBRUSHID_REPEAT		= 1027556,			///< Repeat brush ID.
	SCULPTBRUSHID_AMPLIFY		= 1029169,			///< Amplify brush ID.
	SCULPTBRUSHID_SELECT		= 1031429				///< Select brush ID.
} ENUM_END_LIST(SCULPTBRUSHID);
/// @}
...
{
	FIRSTHITPPOINTTYPE_SELECTED,			///< Use the exact hit location on the surface as the hit point.
	FIRSTHITPPOINTTYPE_AVERAGE,				///< Use the average location of all the points that the brush dab touches.
...
{
	SCULPTBRUSHMODE_NORMAL,			///< Samples the surface as the user moves over it the SculptObject and returns a new hit point and normal each time.
	SCULPTBRUSHMODE_GRAB,				///< Calculates affected points, hit point and normal the first time the user clicks.\n
...
{
	SCULPTBRUSHDATATYPE_NONE				= 0,					///< None.
	SCULPTBRUSHDATATYPE_POINT				= 1 << 0,			///< The points are changed.
	SCULPTBRUSHDATATYPE_MASK				= 1 << 1,			///< The mask data is changed.
	SCULPTBRUSHDATATYPE_COLOR				= 1 << 2,			///< @markInternal
	SCULPTBRUSHDATATYPE_DIRTYPOINTS	= 1 << 3,			///< @markInternal Used by tiles to check if all its points have already been marked as dirty or not.
...
enum OVERRIDE
{
	OVERRIDE_INVERT			= 1 << 0,			///< If the user holds down the @em Ctrl key then the brush should Invert its functionality.
	OVERRIDE_SMOOTH			= 1 << 1,			///< The user is holding down the @em Shift key and a smooth operation should be done.\n
																		///< Brushes can choose to specify their own operation for smooth, like the Mask Tool which does a blur.
	OVERRIDE_SMOOTHTOOL	= 1 << 2			///< @markInternal Used to specify that the smooth is tool specific.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// If the brush mode is ref SCULPTBRUSHMODE_GRAB then this method can be used to get the location of the mouse pointer in world space in the scene.
	/// @return												If the brush mode is @ref SCULPTBRUSHMODE_GRAB returns the position of the mouse in world space, otherwise @c Vector(0,0,0).
	//----------------------------------------------------------------------------------------
	...
	/// @note If the mode is set to @ref SCULPTBRUSHMODE_GRAB then this always returns the initial hit point that occurred when the user first pressed the left mouse button down.
	...
	/// @note	If the mode is set to @ref SCULPTBRUSHMODE_GRAB then this always returns the initial hit polygon that occurred when the user first pressed the left mouse button down.
	/// @since R17
	...
	/// @param[in] offset							The vector to offset the point by.
	/// @param[in] flags							The flags to let use or ignore settings such as the layers mask or strength when offsetting the point: @enumerateEnum{SCULPTOFFSETFLAGS}\n
	/// 															By setting the @ref SCULPTOFFSETFLAGS_IGNOREMASK flag it ignores whatever the mask checkbox state is for the layer.\n
	/// 															Otherwise it uses this state to determine if it should use the mask.
	//----------------------------------------------------------------------------------------
...
public:
	...
	/// @note Check for this in the @c MovePointFunc method if <tt>(GetBrushOverride() & OVERRIDE_SMOOTHTOOL)</tt> is @formatConstant{true} and then implement the custom smooth method.
	...
	/// @param[in] type								The flood data type. Default is @ref SCULPTBRUSHDATATYPE_POINT.
	//----------------------------------------------------------------------------------------
	void SetFloodType(SCULPTBRUSHDATATYPE type);

	//----------------------------------------------------------------------------------------
	/// Specifies the type of brush (grab or normal).
	/// @param[in] mode								The brush mode. Default is @ref SCULPTBRUSHMODE_NORMAL.
	//----------------------------------------------------------------------------------------
	void SetBrushMode(SCULPTBRUSHMODE mode);

	//----------------------------------------------------------------------------------------
	/// Specifies what should happen when the user first clicks on model before dragging.
	/// @param[in] type								The first hit point type. Default is @ref FIRSTHITPPOINTTYPE_SELECTED.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] type								The brush data type. Default is @ref SCULPTBRUSHDATATYPE_POINT.
	//----------------------------------------------------------------------------------------
	...
	/// So you can set all the flags except the @ref DIRTYFLAGS_SELECT (using something like @c (DIRTYFLAGS_ALL & ~DIRTYFLAGS_SELECT)
	...
	/// @since R17
	/// @param[in] flags			The dirty flags to check. By default this is set to @ref DIRTYFLAGS_ALL.
	//----------------------------------------------------------------------------------------
...
public:
	...
	///
	/// Setting SculptBrushParams::SetFloodType() defines what data is affected by the flood. This can be either @ref SCULPTBRUSHDATATYPE_POINT or @ref SCULPTBRUSHDATATYPE_MASK.
	...
	/// This is also called when the Interactive Flood slider is used. Add this slider to the brush by adding the same UI as the Inflate brush has (@em toolsculptbrushinflate.res).
	///
	/// Setting SculptBrushParams::SetFloodType() will define what data is being affected by the flood. This can be either @ref SCULPTBRUSHDATATYPE_POINT or @ref SCULPTBRUSHDATATYPE_MASK.
	/// Setting SculptBrushParams::EnableFlood() to @formatConstant{true} indicates that this method is available to be used, the Flood button will then be active in the UI.
	/// @param[in] pSculpt						The SculptObject.
	/// @param[in] pLayer							The layer to flood.
	/// @param[in] brushData					The brushes BaseContainer data.
...
/// @param[in] mode								The mode for the modifier. Note that @ref SCULPTBRUSHMODE_NORMAL modes can not use @ref SCULPTBRUSHMODE_GRAB modes as modifiers.
