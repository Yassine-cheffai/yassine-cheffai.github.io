...
{
	return (COLORMODE)C4DOS.Bm->MPB_GetParameter((const MultipassBitmap*)this, MPBTYPE_COLORMODE).GetInt32();
}
...
IMAGERESULT BaseBitmap::Init(BaseBitmap*& res, const Filename& name, Int32 frame, Bool* ismovie, BitmapLoaderPlugin** loaderplugin)
{
...
void MovieLoader::FreeData(void)
{
	if (plugin)
	{
		plugin->BmLoadAnimated(&plugin_data, BITMAPLOADERACTION_FREE, nullptr, 0);
		plugin = nullptr;
	}
...
IMAGERESULT MovieLoader::Open(const Filename& fn)
{
	...
	if ((result != IMAGERESULT_OK) || (plugin == nullptr))
	{
		if ((plugin == nullptr) && (result == IMAGERESULT_OK))	// this shouldn't happen
			result = IMAGERESULT_WRONGTYPE;
	...
	if (is_movie)
	{
		...
		result = plugin->BmLoadAnimated(&plugin_data, BITMAPLOADERACTION_INIT, nullptr, 0);
...
// _result:						used to return the result code (IMAGERESULT_OK or error code), can be nullptr
//----------------------------------------------------------------------------------------
BaseBitmap* MovieLoader::Read(Int32 new_frame_idx, Int32* _result)
{
	...
	result = plugin->BmLoadAnimated(&plugin_data, BITMAPLOADERACTION_LOAD, bm, new_frame_idx);	// read the requested frame
	if (result == IMAGERESULT_OK)
	{
...
void BitmapSaverPlugin::BmGetDetails(Int32* alpha, String* suffix)
{
	if (alpha)
	{
		BaseContainer bc;
...
{
	Int32 maxres = BmGetMaxResolution((savebits & SAVEBIT_MULTILAYER) != SAVEBIT_0);
...
AutoBitmap::AutoBitmap(const String& str, Float pixelRatio)
{
	bmp = BaseBitmap::Alloc();
	if (!bmp)
		return;
	if (bmp->Init(GeGetPluginResourcePath() + str) != IMAGERESULT_OK)
	{
		BaseBitmap::Free(bmp);
		return;
	}
