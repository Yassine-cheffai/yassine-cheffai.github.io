...
{
	return C4DOS.Bo->GetMode(this, OBJECTSTATE_EDITOR);
}

void BaseObject::SetEditorMode(Int32 mode)
{
	C4DOS.Bo->SetMode(this, OBJECTSTATE_EDITOR, mode);
}

Int32 BaseObject::GetRenderMode(void)
{
	return C4DOS.Bo->GetMode(this, OBJECTSTATE_RENDER);
}

void BaseObject::SetRenderMode(Int32 mode)
{
	C4DOS.Bo->SetMode(this, OBJECTSTATE_RENDER, mode);
}

Bool BaseObject::GetDeformMode(void)
{
	return C4DOS.Bo->GetMode(this, OBJECTSTATE_DEFORM) != 0;
}

void BaseObject::SetDeformMode(Bool mode)
{
	C4DOS.Bo->SetMode(this, OBJECTSTATE_DEFORM, mode);
}
...
SPLINETYPE SplineObject::GetInterpolationType(void) const
{
	...
	if (GetPointCount() > 2 || type == SPLINETYPE_BEZIER)
		return type;
	else
...
Bool DisjointMesh(PointObject* op)
{
	...
	if (op->GetType() == Opolygon)
	{
		...
		{
			sttpadr = NewMem(Vector, pcnt);
			if (!sttpadr)
				goto error;
			CopyMemType(stt->GetDataAddressR(), sttpadr, pcnt);
		}

		opadr = NewMem(Vector, pcnt);
		if (!opadr)
			goto error;
		...
		if (maxngon > 0)
		{
			ngonpointmap = NewMemClear(Int32, maxngon * 8);
			if (!ngonpointmap)
				goto error;
		}
	...
	else if (op->GetType() == Oline)
	{
		...
		Segment* osadr = NewMem(Segment, scnt);
		if (!osadr)
		{
			DeleteMem(opadr);
			return false;
...
BaseObject* BaseObject::GetAndCheckHierarchyClone(HierarchyHelp* hh, BaseObject* op, HIERARCHYCLONEFLAGS flags, Bool* dirty, AliasTrans* trans, Bool allchildren)
{
	...
	*dirty = *dirty || CheckCache(hh) || IsDirty(DIRTYFLAGS_DATA);
	if (!(*dirty))
	{
		NewDependenceList();
...
Bool AssetData::Add(const Filename& fn, BaseList2D* bl, Bool netRequestOnDemand)
{
	return C4DOS.Bo->AddTexture(bl, &fn, netRequestOnDemand, this);
}
...
BaseSelect* PolygonObject::GetSelectedEdges(Neighbor* e, EDGESELECTIONTYPE ltype)
{
	...
	if (ltype == EDGESELECTIONTYPE_HIDDEN)
		ope = GetEdgeH();
	else if (ltype == EDGESELECTIONTYPE_SELECTION)
		ope = GetEdgeS();
	else if (ltype == EDGESELECTIONTYPE_PHONG)
		ope = GetPhongBreak();
	if (!ope)
		return nullptr;
...
Bool PolyTriangulate::Triangulate(const Vector64* pvPoints, Int32 lPointCount, const Int32* plSegments, Int32 lSegCnt,
																	CPolygon*& pPolys, Int32& lPolyCount, Int32 lFlags, const Int32* plMap, BaseThread* pThread,
																	Int32 lConstraints, const Int32* plConstrainedEdges)
{
	return C4DOS.Bo->PolyTriangTriangA(this, pvPoints, lPointCount, plSegments, lSegCnt, pPolys, lPolyCount, lFlags,
					 plMap, pThread, lConstraints, plConstrainedEdges);
}
#endif
...
#ifdef MAXON_TARGET_SINGLEPRECISION
Bool PolyTriangulate::TriangulateRelease(const Vector64* pvPoints, Int32 lPointCount, const Int32* plSegments, Int32 lSegCnt,
																				 CPolygon*& pPolys, Int32& lPolyCount, Int32 lFlags, const Int32* plMap, BaseThread* pThread,
																				 Int32 lConstraints, const Int32* plConstrainedEdges)
{
	return C4DOS.Bo->PolyTriangTriangReleaseA(this, pvPoints, lPointCount, plSegments, lSegCnt, pPolys, lPolyCount, lFlags,
					 plMap, pThread, lConstraints, plConstrainedEdges);
}
#endif
