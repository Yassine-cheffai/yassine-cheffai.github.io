...
#define COLOR		255.99										///< Constant to convert from vectors color components to integers.
...
//----------------------------------------------------------------------------------------
Tangent TransformTangent(const Vector& newPos, const Vector& planeNormal, const Vector& position, const Tangent& tangent, TANGENTSIDE tangentSide, TANGENTTRANSFORMFLAG flags = TANGENTTRANSFORMFLAG_BREAK_SCALE);
...
public:
	...
	/// @note Set the @ref CALCHARDSHADOW_TRANSPARENCY flag if transparencies/alphas for in between objects shall be evaluated.\n
	...
	//----------------------------------------------------------------------------------------
	void StatusSetText(const String& str) const { C4DOS.Sh->StatusSetText(this, &str); }
	...
	/// @note To enable this functionality @ref VIDEOPOSTINFO_REQUEST_MOTIONGEOMETRY must be set in the video post.\n
	...
	/// @note To avoid self-intersection, pass the current hit as @formatParam{last_hit}. To include all polygons, set @formatParam{last_hit} to RayHitID().
	/// @warning A shader must not call this function if @ref RAYBIT_GI is set in @formatParam{raybits}.
	/// @param[in] ray								The ray to find the intersection for. @callerOwnsPointed{ray}
	/// @param[in] maxdist						The maximum distance to check for intersections.
	/// @param[in] raydepth						The ray depth.
	/// @param[in] raybits						The ray flags: @enumerateEnum{RAYBIT}
	/// @param[in] last_hit						The last intersection. Pass RayHitID() to include all polygons.
	/// @param[in] oldray							To avoid rays to intersect objects (for instance when sampling a hemisphere and rays of the hemisphere going through the surface).\n
	/// 															If the test is not needed pass @formatConstant{nullptr} otherwise TraceColorDirect() will check against the oldray.
	/// @param[out] si								The surface intersection structure to take the calculated hit data. @callerOwnsPointed{surface intersection}
	/// @return												The color.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] p									The surface point.
	/// @param[in] phongn							The surface normal.
	/// @param[in] orign							The original normal.
	...
	//----------------------------------------------------------------------------------------
	void GetDUDV(const TexData* tex, const Vector64& p, const Vector64& phongn, const Vector64& orign, const RayHitID& hit, Bool forceuvw, Vector* ddu, Vector* ddv, Bool usetangents) const { C4DOS.Sh->GetDUDV(this, tex, p, phongn, orign, hit, forceuvw, ddu, ddv, usetangents); }
	...
	/// @since R16.038
	/// @warning Not valid during Videopost execution, with the exception of @ref VIDEOPOSTCALL_TILE.
	...
	/// @param[out] renderThreadIndex	Assigned the index of the render thread that invoked @ref VIDEOPOSTCALL_TILE and 'owns' the tile.\n
...
//----------------------------------------------------------------------------------------
/// A rendering bitmap buffer. Represents internally the same class as MultipassBitmap, so a VPBuffer can be casted to a MultipassBitmap and vice versa.
//----------------------------------------------------------------------------------------
class VPBuffer
{
private:
...
public:
	...
	/// @return												The width.
	//----------------------------------------------------------------------------------------
	Int32 GetBw() const { return C4DOS.Sh->VPGetInfo(this, VPGETINFO_XRESOLUTION); }

	//----------------------------------------------------------------------------------------
	/// Gets the height of the videopost buffer.
	/// @return												The height.
	//----------------------------------------------------------------------------------------
	Int32 GetBh() const { return C4DOS.Sh->VPGetInfo(this, VPGETINFO_YRESOLUTION); }
	...
	/// 															- 32 (::Float)
	//----------------------------------------------------------------------------------------
	Int32 GetBt() const { return C4DOS.Sh->VPGetInfo(this, VPGETINFO_BITDEPTH); }

	//----------------------------------------------------------------------------------------
	/// Gets the bytes per pixel for the videopost buffer.
	/// @return												The number of bytes per pixel.
	//----------------------------------------------------------------------------------------
	Int32 GetCpp() const { return C4DOS.Sh->VPGetInfo(this, VPGETINFO_CPP); }
...
/// @note Buffers that were requested by plugins can be existent, but invisible (not listed in the Picture Viewer %Multipass menu).
	...
	//----------------------------------------------------------------------------------------
	Bool GetVisibleBit() const { return C4DOS.Sh->VPGetInfo(this, VPGETINFO_VISIBLE); }
};
...
public:
	...
	/// @warning The returned volume data is only valid in @ref VIDEOPOSTCALL_INNER.
...
/// @param[in] nodif							Must be initialized with @formatConstant{false} before the call. Assigned true if @formatParam{lr} specifies that @formatParam{op} should not receive any diffuse light.
/// @param[in] nospec							Must be initialized with @formatConstant{false} before the call. Assigned true if @formatParam{lr} specifies that @formatParam{op} should not receive any specular light.
//----------------------------------------------------------------------------------------
...
enum BAKE_STATE
{
	BAKE_STATE_NONE					= 0,					///< None.
	BAKE_STATE_PREPARE			= 10000,			///< Prepare.
	BAKE_STATE_GI_PREPASS		= 10001,			///< GI prepass.
	BAKE_STATE_FILL_IMAGE		= 10002,			///< Fill image.
	BAKE_STATE_COMPLETE			= 10003,			///< Complete.
	BAKE_STATE_INITIALIZE		= 10004,			///< Initialize.

	BAKE_STATE_RESIZENOTIFY = 19999				///< Resize notify. See BakeTextureResizeNotify.
...
struct BakeProgressInfo
{
	...
	void*				data;							///< Only used in case of a @ref BAKE_STATE_RESIZENOTIFY message, cast to @ref BakeTextureResizeNotify*.
...
/// 															Multipass bitmaps must be used if @ref BAKE_TEX_AMBIENT_OCCLUSION is set. The only allowed modes for multipass bitmaps are @ref COLORMODE_RGB, @ref COLORMODE_ARGB, @ref COLORMODE_RGBw, @ref COLORMODE_ARGBw, @ref COLORMODE_RGBf, @ref COLORMODE_ARGBf.
...
/// @return												BAKE_TEX_ERR_NONE if successful, otherwise one of these errors: @enumerateEnum{BAKE_TEX_ERR}
//----------------------------------------------------------------------------------------
