...
#define CAMERA_CUSTOMLENS		4								///< Custom lens projection (see @ref VIDEOPOSTINFO_CUSTOMLENS).
#define CAMERA_CUSTOMLENS_LENSDISTORT 5			///< Custom lens projection used to implement Lens Distortion. @since R17
/// @}
...
#define OBJECTRESTRICTION_TRANSPARENCY_INUSE (1 << 3)			///< Transparency in use.
/// @}
...
public:
	...
	Matrix									motion_delta;					///< Motion transformation matrix. This is <tt>oldmg * !newmg</tt>. The delta time is always 1/fps.
																								///< @note The @ref VIDEOPOSTINFO_REQUEST_MOTIONMATRIX flag must be set for this value to be calculated.
...
public:
	...
	Float							aspectratio;							///< The Y distortion, to get the resulting matrix system multiply by @c m.v2.
	...
	Bool							areanoiselock_ex;					///< @markDeprecated RayParameter::lock_noise is used instead.
...
public:
	...
	Bool				infinite_ex;							///< @markDeprecated
...
public:
	RayHemiSample*									samplesEx;			///< @markDeprecated
	...
	maxon::BaseArray<RayHemiSample>	samples;				///< [READ ONLY] Indirect illumination samples. @since R17
};
...
struct RayRadiancePoly
{
	...
	maxon::BaseArray<RadiancePolyFrontBack>	map_front_back;			///< Radiance values for the front and back of the polygon. @since R17
	Int32																		size_u;							///< Size U.
	Int32																		size_v;							///< Size V.
};
...
struct RayRadianceObject
{
	...
	maxon::BaseArray<RayRadiancePoly> polys;	///< Radiance values buffer for the object's polygons. @since R17
};
...
struct IlluminanceSurfacePointData
{
	...
	Bool													receive_gi,							///< Receive GI.
																receive_caustics,				///< Receive caustics.
