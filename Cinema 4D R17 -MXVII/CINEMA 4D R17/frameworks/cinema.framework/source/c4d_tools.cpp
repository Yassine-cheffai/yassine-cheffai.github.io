...
Float PointLineSegmentDistance2D(const Vector &segmentPoint1, const Vector &segmentPoint2, const Vector &pos, Vector *intersectionPoint, Float *lineOffset)
{
	...
	if (d <= 0.0)
	{
		...
		if (lineOffset)
			*lineOffset = 0.0;
		return Len(Vector(pos.x - segmentPoint1.x, pos.y - segmentPoint1.y, 0.0));
	}
	else if (d >= vl)
	{
		if (intersectionPoint)
		...
		if (lineOffset)
			*lineOffset = 1.0;
		return Len(Vector(pos.x - segmentPoint2.x, pos.y - segmentPoint2.y, 0.0));
	}
	...
	return Len(Vector(pp.x - pos.x, pp.y - pos.y, 0.0));
}
