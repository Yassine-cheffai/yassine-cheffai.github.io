...
{
	return C4DOS.Bd->GetAllTexturesEx(this, ar);
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_USERCHANGE), dat, DESCFLAGS_GET_0))
		return false;
	return dat.GetInt32();
}
...
{
	this->SetParameter(DescLevel(DOCUMENT_NAME), fn, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetDocumentPath(const Filename& fn)
{
	this->SetParameter(DescLevel(DOCUMENT_PATH), fn, DESCFLAGS_SET_DONTCHECKMINMAX);
}

Filename BaseDocument::GetDocumentName(void)
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_NAME), dat, DESCFLAGS_GET_0))
		return Filename();
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_PATH), dat, DESCFLAGS_GET_0))
		return Filename();
	return dat.GetFilename();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_LOD), dat, DESCFLAGS_GET_0))
		return 0.0;
	return dat.GetFloat();
}
...
{
	GeData dat;
	if (!((BaseDocument*)this)->GetParameter(DescLevel(DOCUMENT_FPS), dat, DESCFLAGS_GET_0))
		return 0;
	return dat.GetInt32();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_MINTIME), dat, DESCFLAGS_GET_0))
		return BaseTime();
	return dat.GetTime();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_MAXTIME), dat, DESCFLAGS_GET_0))
		return BaseTime();
	return dat.GetTime();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_LOOPMINTIME), dat, DESCFLAGS_GET_0))
		return BaseTime();
	return dat.GetTime();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_LOOPMAXTIME), dat, DESCFLAGS_GET_0))
		return BaseTime();
	return dat.GetTime();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_RENDERLOD), dat, DESCFLAGS_GET_0))
		return 0;
	return dat.GetInt32();
}
...
{
	this->SetParameter(DescLevel(DOCUMENT_LOD), lod, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetFps(Int32 fps)
{
	this->SetParameter(DescLevel(DOCUMENT_FPS), fps, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetMinTime(const BaseTime& t)
{
	this->SetParameter(DescLevel(DOCUMENT_MINTIME), t, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetMaxTime(const BaseTime& t)
{
	this->SetParameter(DescLevel(DOCUMENT_MAXTIME), t, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetLoopMinTime(const BaseTime& t)
{
	this->SetParameter(DescLevel(DOCUMENT_LOOPMINTIME), t, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetLoopMaxTime(const BaseTime& t)
{
	this->SetParameter(DescLevel(DOCUMENT_LOOPMAXTIME), t, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetRenderLod(Bool lod)
{
	this->SetParameter(DescLevel(DOCUMENT_RENDERLOD), lod, DESCFLAGS_SET_DONTCHECKMINMAX);
}

BaseTime BaseDocument::GetTime(void)
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_TIME), dat, DESCFLAGS_GET_0))
		return BaseTime();
	return dat.GetTime();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_MODE), dat, DESCFLAGS_GET_0))
		return 0;
	return dat.GetInt32();
}
...
{
	GeData dat;
	if (!this->GetParameter(DescLevel(DOCUMENT_ACTION), dat, DESCFLAGS_GET_0))
		return 0;
	return dat.GetInt32();
}
...
{
	this->SetParameter(DescLevel(DOCUMENT_TIME), t, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetMode(Int32 m)
{
	this->SetParameter(DescLevel(DOCUMENT_MODE), m, DESCFLAGS_SET_DONTCHECKMINMAX);
}

void BaseDocument::SetAction(Int32 a)
{
	this->SetParameter(DescLevel(DOCUMENT_ACTION), a, DESCFLAGS_SET_DONTCHECKMINMAX | DESCFLAGS_SET_FORCESET);
}
...
BaseDocument* LoadDocument(const Filename& name, SCENEFILTER loadflags, BaseThread* thread, String* errorString)
{
...
Bool MergeDocument(BaseDocument* doc, const Filename& name, SCENEFILTER loadflags, BaseThread* thread, String* errorString)
{
	return C4DOS.Ge->LoadDocument(doc, name, loadflags | SCENEFILTER_MERGESCENE, thread, nullptr, errorString);
}
...
{
	return C4DOS.Ge->GetAllAssets(doc, allowDialogs, assets, lastPath);
}
...
BaseDocument* BaseDocument::Polygonize(Bool keepanimation)
{
	...
	if (!CopyTo(dst, COPYFLAGS_0, nullptr))
		goto error;
...
Bool InteractiveModeling_Rewind(BaseDocument* doc)
{
	...
	if (!doc || !active)
		return true;
	doc->GetActiveObjects(*active, GETACTIVEOBJECTFLAGS_CHILDREN);
	mu.arr = active;
	mu.doc = doc;
