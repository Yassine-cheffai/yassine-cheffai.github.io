...
typedef Bool MemoryBool;
template <typename T> class Result
{
	static_assert(std::is_same<T, void>::value, "This stub supports void only.");
public:
	Result(const MemoryBool& src = FAILED) : _value(src) {}
	operator MemoryBool& () { return _value; }
...
public:
	...
	T* Append()
	{
		// No null check needed because placement new does this implicitely (see defaultallocator.h for details).
		return new (AppendWithoutConstructor()) T();
	}
	...
	{
		// No null check needed because placement new does this implicitely (see defaultallocator.h for details).
		return new (AppendWithoutConstructor()) T(std::move(x));
	}
	...
	{
		// No null check needed because placement new does this implicitely (see defaultallocator.h for details).
		return new (InsertWithoutConstructor(position)) T();
	}
	...
	{
		// No null check needed because placement new does this implicitely (see defaultallocator.h for details).
		return new (InsertWithoutConstructor(position)) T(std::move(x));
	}
...
protected:
	Bool FitToSize(Int newCapacity)
	{
		if (newCapacity >= _cnt)
		{
			...
			T* newArray = newCapacity > 0 ? (T*) this->Alloc(newCapacity * SIZEOF(T), C4D_MISC_ALLOC_LOCATION) : (T*) this;
			if (newArray)
			{
