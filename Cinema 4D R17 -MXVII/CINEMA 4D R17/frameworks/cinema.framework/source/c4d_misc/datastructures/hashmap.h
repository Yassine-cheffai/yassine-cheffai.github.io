...
//			table = allocator.template AllocArray<Bucket>(c);
		_table = (Bucket*) _allocator.Alloc((Int) c * SIZEOF(Bucket), C4D_MISC_ALLOC_LOCATION);
		if (!_table)
		{
			return false;
		}
...
//			nonEmptyBuckets = allocator.template AllocArray<Int>(c);
		_nonemptyBuckets = (Bucket**) _allocator.Alloc((Int) c * SIZEOF(Bucket*), C4D_MISC_ALLOC_LOCATION);
		if (!_nonemptyBuckets)
		{
...
//			e = allocator.template AllocType<Entry>();
		...
		if (++_size > _resizeThreshold)
		{
			...
			Bucket* t = (Bucket*) _allocator.Alloc(SIZEOF(Bucket) * Int(tlM1 + 1), C4D_MISC_ALLOC_LOCATION);
			if (t)
			{
				ClearMemType(t, Int(tlM1 + 1));
				//				Int* neb = allocator.template AllocArray<Int>(tlM1 + 1);
				Bucket** neb = (Bucket**) _allocator.Alloc(SIZEOF(Bucket*) * Int(tlM1 + 1), C4D_MISC_ALLOC_LOCATION);
				if (!neb)
				{
					_allocator.Free(t);
				}
