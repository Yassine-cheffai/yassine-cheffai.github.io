...
public:
	...
	{
		return new (Insert(position, nullptr, 1)) T();					// no null check needed because placement new does this implicitely (see defaultallocator.h for details)
	}
	...
	{
		return new (Insert(position, nullptr, 1)) T(std::move(x)); // no null check needed because placement new does this implicitely (see defaultallocator.h for details)
	}
...
private:
	...
	struct ForwardAllocator
	{
		...
		void* Alloc(Int s, C4D_MISC_ALLOC_LOCATION_DECLARATION)
		{
			return _a.Alloc(s, C4D_MISC_ALLOC_LOCATION_FORWARD);
		}
		void* Realloc(void* p, Int n, C4D_MISC_ALLOC_LOCATION_DECLARATION)
		{
			return _a.Realloc(p, n, C4D_MISC_ALLOC_LOCATION_FORWARD);
		}
	...
	public:
		...
		Block* Append()
		{
			Block* block = new (this->AppendWithoutConstructor()) Block(*this);
		...
		{
			Block* block = new (this->InsertWithoutConstructor(position)) Block(*this);
