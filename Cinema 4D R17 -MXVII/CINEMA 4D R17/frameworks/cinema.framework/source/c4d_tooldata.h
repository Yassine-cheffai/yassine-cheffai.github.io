...
public:
	...
	/// win->MouseDragStart(button, mx, my, MOUSEDRAGFLAGS_NOMOVE);
	...
	/// @code
	/// while (win->MouseDrag(&dx, &dy, &channels)==MOUSEDRAGRESULT_CONTINUE)
	/// {
	...
	/// @return												@trueIfOtherwiseFalse{the hotkey is pressed down}
	//----------------------------------------------------------------------------------------
...
#define PLUGINFLAG_TOOL_SWITCHACTION				(1 << 13)									///< Allow the tool to switch to a different tool on modifier. @since R17
/// @}
...
class ToolData : public BaseData
{
	...
	friend Bool RegisterPyToolPlugin(Int32 id, const String& str, Int32 info, BaseBitmap* icon, const String& help, ToolData* dat, GeResource *res);
