...
/// @param[out] ports							The port array to destruct. If the pointer is @formatConstant{nullptr} nothing happens. The pointer is assigned @formatConstant{nullptr} afterwards.
/// @param[out] nr_of_ports				The size of @formatParam{ports}. Assigned @em 0.
//----------------------------------------------------------------------------------------
...
/// @param[in] bn									The node. @callerOwnsPointed{GvNode}
/// @param[in] ports							Assigned a pointer to a ports table. @callerOwnsPointed{array}
/// @param[in] nr_of_ports				Assigned the number of @formatParam{ports}.
...
/// @param[in] bn									The node. @callerOwnsPointed{GvNode}
/// @param[out] ports							Assigned a pointer to a ports table. @callerOwnsPointed{array}
/// @param[out] nr_of_ports				Assigned the number of @formatParam{ports}.
/// @return												@trueIfOtherwiseFalse{successful}
//----------------------------------------------------------------------------------------
...
/// @param[in] bn									The node. @callerOwnsPointed{GvNode}
/// @param[out] in_ports					Assigned a pointer to a ports table for the input ports. @callerOwnsPointed{array}
/// @param[out] nr_of_in_ports		Assigned the number of ports in @formatParam{in_ports}.
/// @param[out] out_ports					Assigned a pointer to a ports table for the output ports. @callerOwnsPointed{array}
/// @param[out] nr_of_out_ports		Assigned the number of ports in @formatParam{out_ports}.
/// @return												@trueIfOtherwiseFalse{successful}
//----------------------------------------------------------------------------------------
...
/// @markInternal
//----------------------------------------------------------------------------------------
class OperatorArray : public maxon::SortedArray<OperatorArray, maxon::BaseArray<C4DAtomGoal*, 4> >
{
public:
	OperatorArray()
	{
	...
	MAXON_OPERATOR_MOVE_ASSIGNMENT(OperatorArray)
...
struct DescIdInfo
{
	...
	MAXON_OPERATOR_MOVE_ASSIGNMENT(DescIdInfo)
...
public:
	DescIdInfo* Find(const DescID& key, DescIdInfo& tmp) const
	{
		...
		for (Int32 i = 0; i < GetCount(); i++)
		{
			...
			{
				tmp._id = key;
				if (tmp._name.Content())
					tmp._name += ", ";
				tmp._name += (*this)[i]._name;
				for (Int32 j = 0; j <(*this)[i]._operators.GetCount(); j++)
				{
					if (!tmp._operators.FindValue((*this)[i]._operators[j]))
					{
						if (tmp._operators.Append((*this)[i]._operators[j]) == nullptr)
							return nullptr;
					}
				}
