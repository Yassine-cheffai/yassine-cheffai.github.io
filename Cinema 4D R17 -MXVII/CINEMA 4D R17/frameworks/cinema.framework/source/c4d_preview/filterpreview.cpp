...
// tile:							bitmap tile
//----------------------------------------------------------------------------------------
inline UChar BLEND_WHITE(UChar v)	{ return ((v << 2) - v + 0xffL) >> 2; }
inline UChar BLEND_BLACK(UChar v) { return ((v << 2) - v + 0x80L) >> 2; }

inline UInt16 BLEND_WHITE(UInt16 v)	{ return ((v << 2) - v + (0xffL << 8)) >> 2; }
inline UInt16 BLEND_BLACK(UInt16 v) { return ((v << 2) - v + (0x80L << 8)) >> 2; }
...
static void	darken_bmp(BM_REF bm, BaseBitmap* bmp)
{
	...
	line = NewMemClear(UChar, width);	// but the line width has to take the extra bytes into account (otherwise GetLine overwrites memory)
	if (line)
	{
		Int32	y;
