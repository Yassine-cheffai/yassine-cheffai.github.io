...
Bool StereoVideoPost::StereoMergeImages(BaseVideoPost* node, BaseBitmap* pDest, const BaseBitmap* const* ppSource, Int32 lCount, const BaseContainer& bcSettings, COLORSPACETRANSFORMATION transform)
{
	...
	prLines = NewMem(Float32, 3 * lLineSize);
	if (!prLines)
	{
		return false;
	}
	...
	for (y = 0; y < lHeight; y++)
	{
		ppSource[0]->GetPixelCnt(0, y, lWidth, (UChar*)prLine1, 3 * sizeof(Float32), COLORMODE_RGBf, PIXELCNT_0);
		ppSource[1]->GetPixelCnt(0, y, lWidth, (UChar*)prLine2, 3 * sizeof(Float32), COLORMODE_RGBf, PIXELCNT_0);
		if (lCount == 3)
			ppSource[2]->GetPixelCnt(0, y, lWidth, (UChar*)prLine3, 3 * sizeof(Float32), COLORMODE_RGBf, PIXELCNT_0);
		for (x = 0; x < lWidth; x++)
		{
			prLine1[3 * x + 1] = prLine2[3 * x + 1];
			prLine1[3 * x + 2] = prLine3[3 * x + 2];
		}
		pDest->SetPixelCnt(0, y, lWidth, (UChar*)prLine1, 3 * sizeof(Float32), COLORMODE_RGBf, PIXELCNT_0);
	}
