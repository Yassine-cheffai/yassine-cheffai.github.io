...
inline void SetPixel(COLORMODE colorMode, UInt &idx, UChar *pBuffer, Float32 falloff, UChar *colBuffer, const BaseBitmap *pBitmap = nullptr, int sourceX = 0, int sourceY = 0)
{
	...
	case COLORMODE_GRAY:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 1, COLORMODE_GRAY, PIXELCNT_0);
	...
	case COLORMODE_AGRAY:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 1, COLORMODE_AGRAY, PIXELCNT_0);
			pBuffer[idx] = (UChar)(pBuffer[idx] * (falloffInterp) + colBuffer[0] * falloff);
	...
	case COLORMODE_RGB:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 3, COLORMODE_RGB, PIXELCNT_0);
			pBuffer[idx] = (UChar)(pBuffer[idx] * (falloffInterp) + colBuffer[0] * falloff);
			pBuffer[idx+1] = (UChar)(pBuffer[idx+1] * (falloffInterp) + colBuffer[1] * falloff);
	...
	case COLORMODE_ARGB:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 3, COLORMODE_ARGB, PIXELCNT_0);
			pBuffer[idx] = (UChar)(pBuffer[idx] * (falloffInterp) + colBuffer[0]* falloff);
			pBuffer[idx+1] = (UChar)(pBuffer[idx+1] * (falloffInterp) + colBuffer[1] * falloff);
			pBuffer[idx+2] = (UChar)(pBuffer[idx+2] * (falloffInterp) + colBuffer[2] * falloff);
	...
	case COLORMODE_GRAYw:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 2, COLORMODE_GRAYw, PIXELCNT_0);
	...
	case COLORMODE_AGRAYw:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 2, COLORMODE_AGRAYw, PIXELCNT_0);
			UInt16 *colBuf = (UInt16*)colBuffer;
			UInt16 *destBuf = (UInt16*)&pBuffer[idx];
			destBuf[0] = (UInt16)(destBuf[0] * (falloffInterp) + colBuf[0] * falloff);
	...
	case COLORMODE_RGBw:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 6, COLORMODE_RGBw, PIXELCNT_0);
			UInt16 *colBuf = (UInt16*)colBuffer;
			UInt16 *destBuf = (UInt16*)&pBuffer[idx];
			destBuf[0] = (UInt16)(destBuf[0] * (falloffInterp) + colBuf[0] * falloff);
			destBuf[1] = (UInt16)(destBuf[1] * (falloffInterp) + colBuf[1] * falloff);
	...
	case COLORMODE_ARGBw:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 6, COLORMODE_ARGBw, PIXELCNT_0);
			UInt16 *colBuf = (UInt16*)colBuffer;
			UInt16 *destBuf = (UInt16*)&pBuffer[idx];
			destBuf[0] = (UInt16)(destBuf[0] * (falloffInterp) + colBuf[0] * falloff);
			destBuf[1] = (UInt16)(destBuf[1] * (falloffInterp) + colBuf[1] * falloff);
	...
	case COLORMODE_GRAYf:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 4, COLORMODE_GRAYf, PIXELCNT_0);
	...
	case COLORMODE_AGRAYf:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 4, COLORMODE_AGRAYf, PIXELCNT_0);
			Float32 *colBuf = (Float32*)colBuffer;
			Float32 *destBuf = (Float32*)&pBuffer[idx];
			destBuf[0] = destBuf[0] * (falloffInterp) + colBuf[0] * falloff;
	...
	case COLORMODE_RGBf:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 12, COLORMODE_RGBf, PIXELCNT_0);
			Float32 *colBuf = (Float32*)colBuffer;
			Float32 *destBuf = (Float32*)&pBuffer[idx];
			destBuf[0] = destBuf[0] * (falloffInterp) + colBuf[0] * falloff;
			destBuf[1] = destBuf[1] * (falloffInterp) + colBuf[1] * falloff;
	...
	case COLORMODE_ARGBf:
		{
			if (pBitmap) pBitmap->GetPixelCnt(sourceX, sourceY, 1, (UChar*)colBuffer, 12, COLORMODE_ARGBf, PIXELCNT_0);
			Float32 *colBuf = (Float32*)colBuffer;
			Float32 *destBuf = (Float32*)&pBuffer[idx];
			destBuf[0] = destBuf[0] * (falloffInterp) + colBuf[0] * falloff;
			destBuf[1] = destBuf[1] * (falloffInterp) + colBuf[1] * falloff;
