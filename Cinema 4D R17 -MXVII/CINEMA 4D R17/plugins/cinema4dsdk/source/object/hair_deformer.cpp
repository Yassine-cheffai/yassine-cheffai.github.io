...
public:
	...
	static NodeData* Alloc(void) { return NewObjClear(HairDeformerObject); }
...
static Bool _DeformFn(BaseDocument* doc, BaseList2D* op, HairObject* hair, HairGuides* guides, Vector* padr, Int32 cnt, Int32 scnt)
{
	...
	for (i = 0; i < cnt; i++)
	{
		...
		for (l = 1; l < scnt; l++)
		{
			padr[i * scnt + l] = (tm.TransformVector((padr[i * scnt + l] - padr[i * scnt]))) + padr[i * scnt];
		}
	}
