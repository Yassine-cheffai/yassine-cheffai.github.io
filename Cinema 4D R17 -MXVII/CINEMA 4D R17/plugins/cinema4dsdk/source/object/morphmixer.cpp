...
void MorphMixerObject::MagpieImport(BaseObject* op)
{
	...
	if (!f.FileSelect(FILESELECTTYPE_ANYTHING, FILESELECT_LOAD, GeLoadString(IDS_MORPHSELECT)))
		return;
...
BaseObject* MorphMixerObject::GetVirtualObjects(BaseObject* op, HierarchyHelp* hh)
{
	...
	// clone this object
	ret = (BaseObject*)base->GetClone(COPYFLAGS_NO_HIERARCHY | COPYFLAGS_NO_ANIMATION | COPYFLAGS_NO_BITS, nullptr);
	if (!ret)
		goto error;
